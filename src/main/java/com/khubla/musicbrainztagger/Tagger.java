...
public class Tagger {
   ...
   private void processMP3(File mp3File, String fpcalc, File outputDirectory) throws Exception {
      ...
      if (null != musicbrainzId) {
         ...
         /*
          * we have no idea what this file is. Drop it in the right place, in UNMATCHED
          */
         System.out.println("Unmatched: \"" + mp3File.getName() + "\"");
         /*
          * get the output filename. We just re-root the file to the output dir
          */
         final String outputFileName = outputDir + File.separator + UNMATCHED + mp3File.getAbsolutePath().substring(inputDir.length());
         /*
          * copy
          */
